...
class Cubes {
  ...
  constructor(data:string) {
    ...
    this.value = parseInt(data.split(' ')[0])
  }
}

function isPossible(rounds) {
  const local_pool = {red: 0, green: 0, blue: 0}
  for (const hand of rounds) {
    for (const cubes of hand.map((h)=>new Cubes(h))) {
      if (valid_pool[cubes.color] < cubes.value) {
        return false
      }
      if (local_pool[cubes.color] < cubes.value) {
        local_pool[cubes.color] = cubes.value
      }
    }
  }

  const total_local_pool = Object.values(local_pool).reduce((a, b) => a+b)
  const total_valid_pool = Object.values(valid_pool).reduce((a, b) => a+b)
  return total_local_pool <= total_valid_pool
}
...
let possible_total = 0

for (const game of test_input.split('\n')) {
  const game_data = game.match(/Game ([0-9]+): (.*)/)
  const rounds = game_data[2].split('; ').map((r) => r.split(', '))
  if (isPossible(rounds)) {
    possible_total += parseInt(game_data[1])
  }
}

console.log(possible_total)
