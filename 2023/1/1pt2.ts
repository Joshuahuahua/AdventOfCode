...
function joshsFunction(inputData) {
const input = Array.isArray(inputData) ? inputData.join('\n') : inputData
const numWords = [
  'zero',
  'one',
  'two',
...
// Match FIRST digit (0-9) or number-word (e.g. 'zerone2' = [zero, 2])
const regexpFirst = new RegExp(`[0-9]|${numWords.join('|')}`, 'g')
// Match LAST digit (0-9) or number-word (e.g. 'zerone2' = [one, 2])
const regexpLast = new RegExp(`[0-9]|${numWords.map((num) => `${num}(?!${numWords.filter((word) => word[0] === num[num.length-1]).map((s)=>s.substring(1)).join('|') || ' '})`).join('|')}`, 'g')
// Run both expressions over each line of the input
const nums = input.split("\n").map((s) => [s.match(regexpFirst)[0], s.match(regexpLast).pop()])
// Convert all number-words to integers and combine into a single double-digit number
const digits = nums.map((numArr) => numArr.map((num) => parseInt(num) || numWords.indexOf(num)).join(''))
// Sum list of double-digits
const answer = digits.reduce((a, b) => +a + +b)
console.log(answer)
return digits
}
...
function nelSolution(documents: string[]) {
  const numberDict = {
    zero: "0",
    one: "1",
    two: "2",
  ...
  const numbers = Object.values(numberDict);
  const numberWords = Object.keys(numberDict);
 
  function numberWordToNumber(str: string) {
    const firstWord = numberWords
      .map((word) => ({ word, index: str.indexOf(word) }))
      .filter((item) => item.index !== -1)
      .sort((a, b) => a.index - b.index)
      .map((w) => w.word)[0];
 
    const lastWord = numberWords
      .map((word) => ({ word, index: str.lastIndexOf(word) }))
      .filter((item) => item.index !== -1)
      .sort((a, b) => b.index - a.index)
      .map((w) => w.word)[0];
 
    str = str.replace(firstWord, numberDict[firstWord]);
    str = str.replaceAll(lastWord, numberDict[lastWord]);
    return str;
  }
 
  function findFirstAndLast<T>(arr: T[], fn: (val: T) => boolean): T[] {
    return [arr.find(fn), arr.findLast(fn)];
  }
 
  const sum = documents
    .map((document) => {
      const num = parseInt(
        findFirstAndLast(numberWordToNumber(document).split(""), (char) =>
