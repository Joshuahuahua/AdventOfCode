const data = `.............65..................998.........453...................................845..773.........................307....527...........541
...
const proceedingDigitCount = str.substring(index).match(/^[0-9]+/)[0].length
console.log(index, proceedingDigitCount)
// Get Left-most index
while (parseInt(str[index-1])) {
 index -= 1
}
 const result = parseInt(str.substring(index).match(/^[0-9]+/))
console.log(`${str} | [${index}]:${str[index]} = ${result}`)
 return [result, proceedingDigitCount]
}

function hasSymbolNeighbor(matrix: string[], x:number, y:number) {
  const exp = new RegExp(/[^0-9.x]/)
  ...
  console.log('----------')
  const prevRow = y>0 ? getLeftandRight(matrix[y-1],x) : 'xxx'
  const currRow = getLeftandRight(matrix[y],x)
  const nextRow = y<matrix.length-1 ? getLeftandRight(matrix[y+1],x) : 'xxx'

  console.log(prevRow, exp.test(prevRow))
  console.log(currRow, exp.test(currRow), x)
  console.log(nextRow, exp.test(nextRow))

  return (
    (y>0 && exp.test(getLeftandRight(matrix[y-1],x))) ||          // Row Before
    (exp.test(getLeftandRight(matrix[y],x))) ||                   // Current Row
    (y<matrix.length-1 && exp.test(getLeftandRight(matrix[y+1],x))) // Row After
  )
}
...
function getPartNumSum(input_raw) {
 ...
 const rowSums: number[] = input.map((row, iRow) => {
  ...
  for (let iChar=0; iChar <= row.length; iChar++) {
    // console.log(char, iChar, iRow, hasSymbolNeighbor(input, iChar, iRow))
     if (parseInt(row[iChar]) && hasSymbolNeighbor(input, iChar, iRow)) {
       const [wholeNumber, skip] = getWholeNumber(row, iChar)
       console.log(wholeNumber, skip)
...
console.log(getPartNumSum(test_data))
