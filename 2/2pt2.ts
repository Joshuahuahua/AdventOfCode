...
function totalMinValues(rounds) {
  const local_pool = {red: undefined, green: undefined, blue: undefined}
  for (const hand of rounds) {
    for (const cubes of hand.map((h)=>new Cubes(h))) {
      if (!local_pool[cubes.color] ||cubes.value > local_pool[cubes.color]) {
        local_pool[cubes.color] = cubes.value
      }
    }
  }
  return Object.values(local_pool).reduce((a, b) => a*b)
}

let sumOfPower = 0
for (const game of input.split('\n')) {
  sumOfPower += totalMinValues(game.split(': ')[1].split('; ').map((r) => r.split(', ')))
}
