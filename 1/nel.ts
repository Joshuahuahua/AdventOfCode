...
function numberWordToNumber(str: string) {
  const wordList = numberWords
    .map((word) => ({ word, index: str.indexOf(word) }))
    .filter((item) => item.index !== -1)
    .sort((a, b) => a.index - b.index)
    .map((item) => item.word);
  const last = wordList.length - 1;
  str = str.replace(wordList[0], numberDict[wordList[0]]);
  str = str.replace(wordList[last], numberDict[wordList[last]]);
  console.log(str);
  return str;
}
...
const sum = documents
  .map((document) =>
    parseInt(
      findFirstAndLast(
        numberWordToNumber(numberWordToNumber(document)).split(""),
